...
type CurriedFunction<SuppliedParams extends any[], Fun extends (...args: any[]) => any> = <MoreArgs extends PartialTuple<RemainingParams<SuppliedParams, Parameters<Fun>>>>(
  ...args: MoreArgs
) => CurriedOrValue<[...SuppliedParams, ...MoreArgs], Fun>

type CurriedOrValue<SuppliedParams extends any[], Fun extends (...args: any[]) => any> = RemainingParams<SuppliedParams, Parameters<Fun>> extends [any, ...any[]]
...
export function curry<Fun extends (...param: any[]) => any, InitialParams extends PartialParams<Fun>>(
  ...
  return (...params) => {
    ...
    return totalParams.length >= fnToCurry.length ? fnToCurry(...totalParams) : curry(fnToCurry, ...(totalParams as PartialParams<Fun>))
  }
}
